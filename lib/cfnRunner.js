...
function CFNRunner(region, templatePath) {
    ...
    this.msgPrefix = 'SyncResourcePlugin: ';
    ...
    EventStream(cfn, options.name, {pollInterval: 2000})
        .on('error', function (e) {
            if (stackAction === "DELETE" && e.message.indexOf("does not exist") !== -1) {
                console.log(this.msgPrefix + "Deletion complete.");
                return cb();
            }
            else {
                return cb(e);
        ...
        .on('end', function () {
            console.log(this.msgPrefix + "Starting cleanup...");
            ...
            if (stackAction === "CREATE") {
                cfn.describeStacks({"StackName": options.name}, function (err, data) {
                    if (err) {
                        console.log(this.msgPrefix + "Error getting stack info for cleanup: " + err.cause.message);
                        cb(err);
                    }
                    else {
                        if (data.Stacks.length === 1) {
                            ...
                            console.log(this.msgPrefix + "Stack could not be uniquely identified.  Skipping cleanup...");
                            cb();
                        }
                    }
                        ...
                        .then(function (data) {
                            if (data.Contents.length === 0) {
                                S3.deleteBucketPromised(params)
                                    .then(function () {
                                        console.log(this.msgPrefix + "Ophan buckets deleted.");
                                    })
                                    .catch(function (e) {
                                        console.log(this.msgPrefix + "Error deleting orphan bucket: " + e.cause.message);
            ...
            _this.updateStack(function (err) {
                if (err) {
                    if (err.message === 'No updates are to be performed.') {
                        console.log('CloudFormation: No resource updates are to be performed.');
                    }
                    else {
                        console.error(err);
                        return cb(err);
